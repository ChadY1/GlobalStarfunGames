...
- **hikabrain-plugin** : configurez l'unique arène (lobby, spawn rouge/bleu) dans `hikabrain-plugin/src/main/resources/config.yml` puis déployez-le avec le JAR. La section `bungeecord` permet de rediriger `/hikabrain join` vers un autre serveur.
...
3. **HikaBrain** (StarfunHikaBrain) : copiez `config.yml`, validez le lobby et les spawns. Configuration en jeu : `/hikabrain set <arene> lobby|redspawn|bluespawn` puis `/hikabrain start|stop|join` (permission `fancraft.hikabrain.admin`).
...
4. Démarrez le serveur et vérifiez les commandes `/rush`, `/hikabrain`, `/skywars`, `/freecube`. Sur un cluster BungeeCord, activez `bungeecord.enabled` dans chaque `config.yml` et utilisez `/starfun send <jeu>` depuis le lobby pour basculer vers le serveur de mini‑jeu correspondant.
