...
public class HikaBrainPlugin extends JavaPlugin implements StarfunService {
    ...
    private java.util.Map<String, String> bungeeRoutes = new java.util.HashMap<>();
    ...
    public void onEnable() {
        ...
        if (getConfig().getConfigurationSection("bungeecord.routes") != null) {
            for (String key : getConfig().getConfigurationSection("bungeecord.routes").getKeys(false)) {
                bungeeRoutes.put(key.toLowerCase(), getConfig().getString("bungeecord.routes." + key, ""));
            }
        }
        loadArenas();
        ...
        getCommand("hikabrain").setExecutor(new HikaCommand(this, gameManager, proxyBridge, bungeeEnabled, bungeeRoutes));
