...
public class HikaCommand implements CommandExecutor {
    ...
    private final java.util.Map<String, String> bungeeRoutes;

    public HikaCommand(HikaBrainPlugin plugin, HikaGameManager manager, ProxyBridge proxyBridge, boolean bungeeEnabled, java.util.Map<String, String> bungeeRoutes) {
        this.plugin = plugin;
        this.manager = manager;
        this.proxyBridge = proxyBridge;
        this.bungeeEnabled = bungeeEnabled;
        this.bungeeRoutes = bungeeRoutes;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("join")) {
            String queue = args.length > 1 ? args[1].toLowerCase() : "default";
            String target = bungeeEnabled
                    ? bungeeRoutes.getOrDefault(queue, bungeeRoutes.getOrDefault("default", ""))
                    : "";
            if (target != null && !target.isEmpty()) {
                proxyBridge.connect(player, target);
                sender.sendMessage(ChatColor.AQUA + "Connexion au serveur HikaBrain " + target + "...");
                return true;
            }
